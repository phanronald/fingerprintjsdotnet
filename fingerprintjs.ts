...
module FingerPrints {
	export class FingerPrinting {
		...
		private getWebGlFeatures(): string {
			...
			(<any>vertexPosBuffer).itemSize = 3;
			(<any>vertexPosBuffer).numItems = 3;

			var program: WebGLProgram = gl.createProgram(), vshader = gl.createShader(gl.VERTEX_SHADER);
			...
			(<any>program).vertexPosAttrib = gl.getAttribLocation(program, "attrVertex");
			(<any>program).offsetUniform = gl.getUniformLocation(program, "uniformOffset");
			gl.enableVertexAttribArray((<any>program).vertexPosArray);
			gl.vertexAttribPointer((<any>program).vertexPosAttrib,(<any>vertexPosBuffer).itemSize, gl.FLOAT, !1, 0, 0);
			gl.uniform2f((<any>program).offsetUniform, 1, 1);
			gl.drawArrays(gl.TRIANGLE_STRIP, 0,(<any>vertexPosBuffer).numItems);
		...
		private x64Multiply(m, n): Array<number> {
			m = [m[0] >>> 16, m[0] & 0xffff, m[1] >>> 16, m[1] & 0xffff];
			n = [n[0] >>> 16, n[0] & 0xffff, n[1] >>> 16, n[1] & 0xffff];
			var o = [0, 0, 0, 0];
			o[3] += m[3] * n[3];
		...
		private x64LeftShift(m, n): number[] {
			n %= 64;
			if (n === 0) {
				return m;
			}
		...
		private x64Xor(m, n): Array<number> {
			return [m[0] ^ n[0], m[1] ^ n[1]];
		}

		private x64Fmix(h):number[] {
		...
		private murmurhash3_64_gc(key, seed):string {
			key = key || "";
			seed = seed || 0;
		...
		getFingerPrintHash(): any {
			...
			if (this.useScreenResolution) {
				var resolution: Array<number> = this.getScreenResolution();
				if (typeof resolution !== 'undefined') {
					keys.push(resolution.join('x'));
				}

				var availableResolution: Array<number> = this.getAvailableScreenResolution();
